<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
    <title>Boilerplate - As Roma Updates</title>
    <meta name="description" content="The Imarc Boilerplate is a streamlined starting point for building websites." />
    <meta name="web_author" content="Imarc, A Digital Agency | Boston + Silicon Valley | more info at imarc.com" />
    <meta name="viewport" content="width=device-width" />
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
    <link href="https://fonts.googleapis.com/css?family=Dosis:300,400,700" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="css/styles.css" media="all" />

</head>
<body>
    <div class="wrapper">
        <header class="primary">
            <div class="container">
                <div class="header-content">
                    <div class="branding">
                        <a href="#">
                            <img src="img/logo-boilerplate.png" alt="Imarc Boilerplate">
                        </a>
                    </div>
                    <button class="menu-toggle"><i class="fa fa-bars"></i></button>
                    <div class="mobile">
                        <nav aria-label="Primary Mobile Navigation" class="primary">
                            <ul>
                                <li>
                                    <a href="">Maine</a>
                                    <div class="dropdown">
                                        <ul>
                                            <li><a href="">Augusta</a></li>
                                            <li><a href="">Eliot</a></li>
                                            <li><a href="">Kittery</a></li>
                                            <li><a href="">Portland</a></li>
                                        </ul>
                                    </div>
                                </li>
                                <li class="active">
                                    <a href="#yup">Markup</a>
                                </li>
                                <li>
                                    <a href="">New Hampshire</a>
                                    <div class="dropdown">
                                        <ul>
                                            <li><a href="">Concord</a></li>
                                            <li><a href="">Manchester</a></li>
                                            <li><a href="">Portsmouth</a>
                                                <div class="dropdown">
                                                    <ul>
                                                        <li><a href="#">Main Street</a></li>
                                                        <li><a href="#">Woodbury Ave</a></li>
                                                    </ul>
                                                </div>
                                            </li>
                                            <li><a href="">Hampton</a></li>
                                        </ul>
                                    </div>
                                </li>
                                <li>
                                    <a href="">Vermont</a>
                                    <div class="dropdown">
                                        <ul>
                                            <li><a href="">Montpelier</a></li>
                                            <li><a href="">Burlington</a></li>
                                            <li><a href="">Killington</a></li>
                                            <li><a href="">St. Johnsbury</a></li>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
                        </nav>
                        <div class="utilities">
                            <nav aria-label="Utility Navigation" class="utility">
                                <ul>
                                    <li><a href="">Careers</a></li>
                                    <li><a href="">Legal</a></li>
                                    <li><a href="">Log In</a></li>
                                </ul>
                            </nav>
                            <form class="search" method="post">
                                <div class="text">
                                    <label for="search">Search the website</label>
                                    <input id="search" placeholder="Search the website" type="text" />
                                </div>
                                <div class="submit">
                                    <input type="submit" value="Search" />
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="desktop">
                        <div class="utilities">
                            <nav aria-label="Utility Navigation" class="utility">
                                <ul>
                                    <li><a href="">Careers</a></li>
                                    <li><a href="">Legal</a></li>
                                    <li><a href="">Log In</a></li>
                                </ul>
                            </nav>
                        </div>
                        <nav aria-label="Primary Desktop Navigation" class="primary">
                            <ul>
                                <li class="mega">
                                    <a href="">Main Items <i class="fa fa-caret-down" aria-hidden="true"></i></a>
                                    <div class="dropdown">
                                        <div class="menu">
                                            <div>
                                                <ul>
                                                    <li>
                                                        <a href="">Products</a>
                                                    </li>
                                                    <li>
                                                        <a href="">Services</a>
                                                    </li>
                                                    <li>
                                                        <a href="">Blog</a>
                                                    </li>
                                                    <li>
                                                        <a href="">About Us</a>
                                                    </li>
                                                    <li>
                                                        <a href="">Contact</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div>
                                                <h3>Lorem Ipsum Dolar Amat</h3>
                                                <p>
                                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Blanditiis dignissimos quisquam numquam veniam eos reprehenderit, nemo sunt nihil rem suscipit.
                                                </p>
                                                <p>
                                                    Nemo sunt nihil rem lorem amat fuga consectetur adipisicing elit.
                                                </p>
                                            </div>
                                            <div>
                                                <!--<img src="/img/AS_Roma_logo_.svg" alt=""> -->
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li>
                                    <a href="#yup">Markup <i class="fa fa-caret-down" aria-hidden="true"></i></a>
                                    <div class="dropdown">
                                        <div class="menu">
                                            <div>
                                                <ul>
                                                    <li>
                                                        <a href="">Hello World</a>
                                                    </li>
                                                    <li>
                                                        <a href="">Lorem Ipsum Amat</a>
                                                    </li>
                                                    <li>
                                                        <a href="">HTML is on version 5</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li>
                                    <a href="">New Hampshire</a>
                                </li>
                                <li>
                                    <a href="">Vermont</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                </div>
            </div>
        </header>

        <div class="hero">
            <div class="weather-container">
                <div id="weather"></div>
            </div>
            <div class="copy">
                <h1>
                    As Roma Updates Coming Soon
                </h1>
                <a class="button" href="https://github.com/vibrains/boilerplate">
                    Boilerplate on Github
                </a>
            </div>
        </div>

        <div class="torso">
            <div class="container">
                <aside class="primary">
                    <section>
                        <nav class="supplemental">
                            <ul>
                                <li>
                                    <a href="#Typography">Typography</a>
                                    <ul>
                                        <li><a href="#Typographical-Blocks">Typographical Blocks</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#Lists">Lists</a>
                                    <ul>
                                        <li><a href="#Unordered-Lists">Unordered Lists</a></li>
                                        <li><a href="#Ordered-Lists">Ordered Lists</a></li>
                                        <li><a href="#Definition-Lists">Definition Lists</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#Tables">Tables</a>
                                </li>
                                <li>
                                    <a href="#Forms">Forms</a>
                                    <ul>
                                        <li><a href="#Messaging">Messaging</a></li>
                                        <li><a href="#Fields">Fields</a></li>
                                        <li><a href="#Buttons">Buttons</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#Iconography">Iconography</a>
                                    <ul>
                                        <li><a href="#Dedicated-element">Dedicated Element</a></li>
                                        <li><a href="#Pseudo-element">Pseudo Element</a></li>
                                    </ul>
                                </li>
                            </ul>
                        </nav>
                    </section>
                </aside>

                <main>
                    <ul id="results"></ul>
                    <pre id="response"></pre>
                    <section>
                        <div class="columns">
                            <div class="services">
                                <p>1/3 column</p>
                            </div>
                            <hr>
                            <div class="services">
                                <p>1/3 column</p>
                            </div>
                            <hr>
                            <div class="services">
                                <p>1/3 column</p>
                            </div>
                        </div>
                    </section>

                    <section>

                        <h2>WP Post</h2>
                        <p><strong>How to fix 'Fatal error: Allowed memory size of xxxx bytes exhausted'</strong></p>
                        <p>
                            After working with WordPress for several years, the "Allowed memory size exhausted" error is one of the more common errors that you will run into.
                            So how do I fix it?

                            First and easiest thing to try is to increase the PHP memory limit by adding this line of code to the wp-config.php file like so:<br>

                            <code> define( 'WP_MEMORY_LIMIT', '256M' );</code><br>

                            If this still does not work, try the following:
                            Go to home/username/public_html/wp-includes/default-constants.php
                            Open the file and replace the following:<br>

                            <code><span class="comment">// Define memory limits.</span><br>
                                <span class="ifthen">if</span> ( ! <span class="condition">defined</span>( '<span class="function">WP_MEMORY_LIMIT</span>' ) ) {<br>
                                <span class="ifthen">if</span> ( false === <span class="condition">wp_is_ini_value_changeable(</span> '<span class="function">memory_limit</span>' ) ) {<br>
                                <span class="condition">define</span>( 'WP_MEMORY_LIMIT', $current_limit );<br>
                            } <span class="ifthen">elseif</span> ( <span class="condition">is_multisite() )</span> {<br>
                            <span class="condition">define</span>( 'WP_MEMORY_LIMIT', '<span class="function">64M</span>' );<br>
                        } <span class="ifthen">else</span> {<br>
                        <span class="condition">define</span>( 'WP_MEMORY_LIMIT', '<span class="function">40M</span>' );<br>
                    }
                }</code><br>

                with the following code:<br>

                <code><span class="comment">// Define memory limits.</span><br>
                    <span class="ifthen">if</span> ( ! <span class="condition">defined</span>( '<span class="function">WP_MEMORY_LIMIT</span>' ) ) {<br>
                    <span class="ifthen">if</span> ( false === <span class="condition">wp_is_ini_value_changeable(</span> '<span class="function">memory_limit</span>' ) ) {<br>
                    <span class="condition">define</span>( 'WP_MEMORY_LIMIT', $current_limit );<br>
                } <span class="ifthen">elseif</span> ( <span class="condition">is_multisite() )</span> {<br>
                <span class="condition">define</span>( 'WP_MEMORY_LIMIT', '<span class="function">256M</span>' );<br>
            } <span class="ifthen">else</span> {<br>
            <span class="condition">define</span>( 'WP_MEMORY_LIMIT', '<span class="function">256M</span>' );<br>
        }
    }</code><br>


</p>

<h2 id="Tables">Tables</h2>

<p>
    This table has a caption, colgroup, thead, tfoot, and tbody.
</p>

<table>
    <caption>Table with caption, colgroup, thead, tfoot, and tbody</caption>
    <colgroup>
    <col />
    <col />
    <col />
</colgroup>
<thead>
    <tr>
        <th>header 1</th>
        <th>header 2</th>
        <th>header 3</th>
    </tr>
</thead>
<tfoot>
    <tr>
        <td>footer 1</td>
        <td>footer 2</td>
        <td>footer 3</td>
    </tr>
</tfoot>
<tbody>
    <tr>
        <td>body 1</td>
        <td>body 2</td>
        <td>body 3</td>
    </tr>
    <tr>
        <td>body 1</td>
        <td>body 2</td>
        <td>body 3</td>
    </tr>
    <tr>
        <td>body 1</td>
        <td>body 2</td>
        <td>body 3</td>
    </tr>
    <tr>
        <td>body 1</td>
        <td>body 2</td>
        <td>body 3</td>
    </tr>
</tbody>
</table>
</section>
</main>
</div>
</div>

<?php include('components/footer.php'); ?>

<script type="text/javascript" src="js/jquery.min.js"></script>
<script src="js/mobileNavigation.plugin.js" type="text/javascript"></script>
<script src="js/dropdown.plugin.js"></script>
<script src="js/stickyHeader.plugin.js"></script>
<script src="js/tilt.js"></script>
<script src="js/slick.min.js"></script>
<script src="js/carousel.js"></script>
<script type="text/javascript" src="js/geonames_request.js"></script>
<script type="text/javascript" src="js/football-data-request.js"></script>
<script type="text/javascript" src="js/ripple.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.simpleWeather/3.1.0/jquery.simpleWeather.min.js"></script>

<script>
    $(function () {
        $('header.primary').stickyHeader();
        $('nav.primary').dropdown();
    });
</script>

<script>
    $(function () {
        $('header.primary').stickyHeader();
        $('nav.primary').dropdown();
    });
</script>

<script>
    $.ripple(".button", {
    debug: false, // Turn Ripple.js logging on/off
    on: 'mousedown', // The event to trigger a ripple effect

    opacity: 0.4, // The opacity of the ripple
    color: "#2b2b2b", // Set the background color. If set to "auto", it will use the text color
    multi: false, // Allow multiple ripples per element

    duration: 0.5, // The duration of the ripple

    // Filter function for modifying the speed of the ripple
    rate: function(pxPerSecond) {
        return pxPerSecond;
    },

    easing: 'linear' // The CSS3 easing function of the ripple
});
</script>

<script>
    $('.slick-slider').slick({
      nextArrow: '<i class="fa fa-arrow-right"></i>',
      prevArrow: '<i class="fa fa-arrow-left"></i>',
      dots: true,
      infinite: true,
      speed: 500,
      fade: false,
      cssEase: 'linear'
// add the rest of your options here
});
</script>

<script>
    $(document).ready(function() {
      $.simpleWeather({
        location: 'Rome, IT',
        woeid: '',
        unit: 'f',
        success: function(weather) {
          html = '<h2><i class="icon-'+weather.code+'"></i> '+weather.temp+'&deg;'+weather.units.temp+'</h2>';
          // html += '<ul><li>'+weather.city+', '+weather.region+'</li>';
          // html += '<li class="currently">'+weather.currently+'</li>';
          // html += '<li>'+weather.wind.direction+' '+weather.wind.speed+' '+weather.units.speed+'</li></ul>';

          $("#weather").html(html);
      },
      error: function(error) {
          $("#weather").html('<p>'+error+'</p>');
      }
  });
  });
</script>

</body>
</html>
